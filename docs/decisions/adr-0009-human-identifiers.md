...
Michel proposed accepting the identifiers the world already uses: a phone number with country and area code, a tax ID (unique per person and therefore excellent for government use), an email address, and `user@server`, noting that this last form also serves to hide the identity key behind a managed server.
...
2. **`user@server` remains the handle**, which has existed since ADR-0003 and fulfils exactly the role Michel described: a readable form that hides the key and allows using a managed server.
